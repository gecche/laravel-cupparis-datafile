...
class ArdentDatafileProvider implements DatafileProviderInterface
{
    /*
    ...
    public function __construct()
    {
        ...
        $this->queues_namespace = Config::get('app.queues_namespace') . "\\";
        $this->models_namespace = Config::get('app.models_namespace') . "\\";
        $this->datafilemodels_namespace = Config::get('app.datafilemodels_namespace') . "\\";

        $this->modelDatafileName = $this->datafilemodels_namespace . $this->modelDatafileName;
        $this->modelTargetName = $this->models_namespace . $this->modelTargetName;

        $this->datafileModelErrorName = $this->datafilemodels_namespace . 'Error';
        $reflector = new \ReflectionClass($this->modelDatafileName);
        if (!$reflector->isSubclassOf('\\Cupparis\\Datafile\\ArdentDatafile')) {
            throw new \ReflectionException('Invalid class for model datafile');
        };
        $reflector = new \ReflectionClass($this->modelTargetName);
        if (!$reflector->isSubclassOf('\\Cupparis\\Ardent\\Ardent')) {
...
//        if (!$datafileProvider->hasHeaders()) {
//        $this->datafileHandler->setHeaders($datafileProvider->getPlainHeaders());
//        }
        ...
        foreach ($doubleDatafileErrors as $doubleDatafileError) {
            ...
            if (count($models) > 1) {
                foreach ($models as $currModel) {
                    ...
                    $datafileError = $datafileErrorName::create(array(
...
//                    $not_found[] = $field;
//                } else {
//                    $fileHeaders = array_splice($fileHeaders,$index,1);
//                }
            }
...
//            $msg = '';
//            $msg .=  "Headers da impostare: " . implode(", ",$this->headers) . "\n\n";
//
//            $msg .=  "Headers trovati nel file: " . implode(", ",$fileHeaders) . "\n\n";
