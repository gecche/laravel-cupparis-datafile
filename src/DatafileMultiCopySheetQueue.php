...
class DatafileMultiCopySheetQueue extends MainQueue {

    public function load($job, $data) {
		$this->jobStart ($job,$data, 'datafile_multi_copy_sheet_load' );
		try {
            $this->validateData("datafile_multi_copy_sheet_load");
...
//            Log::info('Datafile provider name: '. $datafileProviderName);
            ...
            $fileTypeProvider = Datafile::getDatafileProvider()->getFiletype();
            if ($fileTypeProvider !== 'excel_multi_copy') {
                throw new \InvalidArgumentException("Datafile provider (". $fileTypeProvider. ") non supportato per questo job");
            }
...
//            Log::info('hereENDENF3');
	...
	public function save($job, $data) {
		$this->jobStart ($job,$data, 'datafile_multi_copy_sheet_save' );

        try {

            $this->validateData("datafile_multi_copy_sheet_save");
	...
	protected function validateData($job_type) {
		...
		if ($job_type == "datafile_multi_copy_sheet_load") {
			if (!Arr::get($this->data, 'fileName',false)) {
				throw new Exception("File datafile non definito!");
			}
		}
		if ($job_type == "datafile_multi_copy_sheet_save") {
